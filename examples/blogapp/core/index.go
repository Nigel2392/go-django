...
var _ (models.ContextSaver) = (*MainStruct)(nil)
...
var _ = contenttypes.Register(&contenttypes.ContentTypeDefinition{
	ContentObject: &MainStruct{},
	GetInstance: func(identifier any) (any, error) {
	...
	GetInstances: func(amount, offset uint) ([]any, error) {
		var items = make([]interface{}, 0, len(mainStructMap))
		...
		slices.SortFunc(items, func(a, b interface{}) int {
			aa := a.(*MainStruct)
			bb := b.(*MainStruct)
			return strings.Compare(attrs.Get[string](aa, "Email"), attrs.Get[string](bb, "Email"))
		})
...
var _ = admin.RegisterApp(
	...
	admin.AppOptions{
		...
		MenuLabel:           trans.S("Core"),
	},
	admin.ModelOptions{
		RegisterToAdminMenu: true,
		// Fields:              []string{"Email", "Name", "Password", "Age", "Data", "Block"},
		Labels: map[string]func() string{
			"Email":    trans.S("Object Email"),
			"Name":     trans.S("Object Name"),
			"Password": trans.S("Object Password"),
			"Age":      trans.S("Object Age"),
			"Data":     trans.S("Object Data"),
			"Block":    trans.S("Object Block"),
		},
		...
		ListView: admin.ListViewOptions{
			Format: map[string]func(interface{}) interface{}{
				...
				"Editor": func(v any) interface{} {
					...
					var data, err = v.(*editor.EditorJSBlockData).Render()
					if err != nil {
						return fmt.Sprintf("Error: %s", err)
					}
			...
			Columns: map[string]list.ListColumn[attrs.Definer]{
				"Editor": list.HTMLColumn[attrs.Definer](
					trans.S("Editor Data"),
					func(r *http.Request, defs attrs.Definitions, row attrs.Definer) template.HTML {
						var data, err = row.(*MainStruct).Editor.Render()
						if err != nil {
