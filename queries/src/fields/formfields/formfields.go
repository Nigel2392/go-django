...
func (f *ForeignKeyFormField) HasChanged(initial, data interface{}) bool {
	var pkA = fkPrimaryKey(initial)
	var pkB = fkPrimaryKey(data)
	return !reflect.DeepEqual(pkA, pkB)
}

// fkPrimaryKey extracts a comparable primary key from a value that may be
// either an attrs.Definer (returning its PK via attrs.PrimaryKey) or a raw
// scalar value (returned as-is).  This lets HasChanged correctly compare an
// initial value stored as a plain integer with a cleaned value that has been
// converted into a full Definer object by ValueToGo.
func fkPrimaryKey(v interface{}) interface{} {
	if d, ok := v.(attrs.Definer); ok {
		return attrs.PrimaryKey(d)
	}
	return v
}
