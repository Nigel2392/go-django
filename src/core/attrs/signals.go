...
var (
	...
	PreResetDefinitions  = signals.New[any]("attrs.pre.ResetDefinitions")
	PostResetDefinitions = signals.New[any]("attrs.post.ResetDefinitions")
	...
	alreadyCalledOnModelRegister = make(map[reflect.Type]struct{})
	...
	_, _ = ResetDefinitions.Listen(context.Background(), func(ctx context.Context, s signals.Signal[any], a any) error {
		...
		for _, n := range graph {
			if n.meta.model == nil {
				panic("meta model is nil")
			}

			modelTyp := reflect.TypeOf(n.meta.model)
			_, alreadyFinal := alreadyCalledOnModelRegister[modelTyp]
			if !alreadyFinal {
				alreadyCalledOnModelRegister[modelTyp] = struct{}{}
			}
			...
			registerModel(
				...
				// final=true inside of this signal ensures that [CanOnModelRegister]
				// interface methods get called exactly ONCE.
				!alreadyFinal,
				false,
			)
		}
		...
		for _, n := range graph {
			...
			n.meta.definitions = newStaticDefinitions(
				NewObject[Definer](
					ContextWithFlags(context.Background(), CtxFlagRegistering, true),
					n.meta.model,
				),
