...
func (b *Backend) GetValidName(name string) string {
	b.mu.RLock()
	defer b.mu.RUnlock()
	...
	var root = name[:len(name)-len(ext)]
	return b.GetAlternateName(root, ext)
}

// Validates the filename by calling GetValidName() and returns a filename to be passed to the Save() method.
func (b *Backend) GenerateFilename(filename string) string {
	return b.GetValidName(filename)
}
