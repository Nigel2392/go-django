...
func NewMySQLStore(db drivers.DB) *MySQLStore {
	return NewMySQLStoreWithCleanupInterval(db, 5*time.Minute)
}
...
func NewMySQLStoreWithCleanupInterval(db drivers.DB, cleanupInterval time.Duration) *MySQLStore {
	m := &MySQLStore{
		DB:      db,
...
func (m *MySQLStore) Find(token string) ([]byte, bool, error) {
	...
	row := m.DB.QueryRowContext(context.Background(), stmt, token)
	err := row.Scan(&b)
...
func (m *MySQLStore) Commit(token string, b []byte, expiry time.Time) error {
	_, err := m.DB.ExecContext(context.Background(), "INSERT INTO sessions (token, data, expiry) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), expiry = VALUES(expiry)", token, b, expiry.UTC())
	if err != nil {
		return err
	}
	return nil
...
func (m *MySQLStore) Delete(token string) error {
	_, err := m.DB.ExecContext(context.Background(), "DELETE FROM sessions WHERE token = ?", token)
	return err
}
...
func (m *MySQLStore) deleteExpired() error {
	...
	_, err := m.DB.ExecContext(context.Background(), stmt)
	return err
}

func getVersion(db drivers.DB) string {
	var version string
