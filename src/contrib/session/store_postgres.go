...
func NewPostgresStore(db drivers.DB) *PostgresStore {
	return NewPostgresStoreWithCleanupInterval(db, 5*time.Minute)
}
...
func NewPostgresStoreWithCleanupInterval(db drivers.DB, cleanupInterval time.Duration) *PostgresStore {
...
func (p *PostgresStore) Find(token string) (b []byte, exists bool, err error) {
	row := p.db.QueryRowContext(context.Background(), "SELECT data FROM sessions WHERE token = $1 AND current_timestamp < expiry", token)
	err = row.Scan(&b)
...
func (p *PostgresStore) Commit(token string, b []byte, expiry time.Time) error {
	_, err := p.db.ExecContext(context.Background(), "INSERT INTO sessions (token, data, expiry) VALUES ($1, $2, $3) ON CONFLICT (token) DO UPDATE SET data = EXCLUDED.data, expiry = EXCLUDED.expiry", token, b, expiry)
	if err != nil {
		return err
	}
	return nil
...
func (p *PostgresStore) Delete(token string) error {
	_, err := p.db.ExecContext(context.Background(), "DELETE FROM sessions WHERE token = $1", token)
	return err
}
...
func (p *PostgresStore) All() (map[string][]byte, error) {
	rows, err := p.db.QueryContext(context.Background(), "SELECT token, data FROM sessions WHERE current_timestamp < expiry")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PostgresStore) deleteExpired() error {
	_, err := p.db.ExecContext(context.Background(), "DELETE FROM sessions WHERE expiry < current_timestamp")
	return err
}
