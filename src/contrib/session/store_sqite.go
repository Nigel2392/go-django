...
func NewSQLiteStore(db drivers.DB) *SQLite3Store {
	return NewSQLiteStoreWithCleanupInterval(db, 5*time.Minute)
}
...
func NewSQLiteStoreWithCleanupInterval(db drivers.DB, cleanupInterval time.Duration) *SQLite3Store {
...
func (p *SQLite3Store) Find(token string) (b []byte, exists bool, err error) {
	row := p.db.QueryRowContext(context.Background(), "SELECT data FROM sessions WHERE token = $1 AND julianday('now') < expiry", token)
	err = row.Scan(&b)
...
func (p *SQLite3Store) Commit(token string, b []byte, expiry time.Time) error {
	_, err := p.db.ExecContext(context.Background(), "REPLACE INTO sessions (token, data, expiry) VALUES ($1, $2, julianday($3))", token, b, expiry.UTC().Format("2006-01-02T15:04:05.999"))
	if err != nil {
		return err
	}
	return nil
...
func (p *SQLite3Store) All() (map[string][]byte, error) {
	rows, err := p.db.QueryContext(context.Background(), "SELECT token, data FROM sessions WHERE julianday('now') < expiry")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *SQLite3Store) deleteExpired() error {
	_, err := p.db.ExecContext(context.Background(), "DELETE FROM sessions WHERE expiry < julianday('now')")
	return err
}
