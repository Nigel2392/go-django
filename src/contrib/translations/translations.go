...
var makeTranslationsCommand = &command.Cmd[translationsCommandContext]{
	...
	Execute: func(m command.Manager, stored translationsCommandContext, args []string) (err error) {
		...
		if translatorApp.translationHeader == nil {
			translatorApp.translationHeader = &FileTranslationsHeader{
		...
		if len(locales.List()) > 0 {
			for _, locale := range locales.List() {
				if _, ok := translatorApp.translationHeader.Locales.Get(locale); ok {
					continue // Locale already exists
				}

				translatorApp.translationHeader.Locales.Set(locale, TranslationHeaderLocale{
					NumPluralForms: 2,
					PluralRule:     "(n != 1)",
				})
			}
		...
		encoder.SetIndent(2)
		if err := encoder.Encode(translatorApp.translationHeader); err != nil {
			return cli.Exit(
