...
func (p *pageRegistry) SpecificInstance(ctx context.Context, node *PageNode) (Page, error) {
	...
	if definition.GetForID == nil {
		...
		var ref = row.Object.Reference()
		if ref == nil {
			// If the reference is nil, we cannot really do anything with it,
			// log a warning and return the row object.
			logger.Warnf(
				"pages: SpecificInstance cannot set reference for node %s with PageID %d, reference pointer is nil",
				typeName, node.PageID,
			)
...
// i really do not like this hack... seems to work for now.
func isPromoted(rTyp reflect.Type, methodName string) bool {
	m, ok := rTyp.MethodByName(methodName)
	if !ok {
		return false
	}
