...
func matchCompile(pattern string) (match *Match, err error) {
	...
	for i := 0; i < len(arr); i++ {
		if escaped {
			...
			case '*', '?', '\\':
				regex += "\\" + string(arr[i])
			default:
				return nil, fmt.Errorf("Invalid escaped character '%c'", arr[i]) //lint:ignore ST1005 This is a custom error message 1
			}
		} else {
			switch arr[i] {
			case '\\':
	...
	if escaped {
		return nil, fmt.Errorf("Unterminated escape at end of pattern") //lint:ignore ST1005 This is a custom error message 2
	}
