...
func (i SliceField[T]) InputHTML(_ *request.Request, name string, tags tags.TagMap) interfaces.Element {
	...
	for _, v := range i {
		b.WriteString(fmt.Sprintf(`<input type="text" name="%s" value="%v" %s>`, name, v, TagMapToElementAttributes(tags, AllTagsInput...)))
	}
...
func (i SelectField) LabelHTML(_ *request.Request, name string, tags tags.TagMap) interfaces.Element {
	return ElementType(fmt.Sprintf(`<label for="%s" %s>%s</label>`, name, TagMapToElementAttributes(tags, AllTagsLabel...), name))
}

func (i SelectField) InputHTML(_ *request.Request, name string, tags tags.TagMap) interfaces.Element {
	var b strings.Builder
	b.WriteString(fmt.Sprintf(`<select name="%s" %s>`, name, TagMapToElementAttributes(tags, AllTagsInput...)))
	for _, v := range i {
...
func (i MultipleSelectField) LabelHTML(_ *request.Request, name string, tags tags.TagMap) interfaces.Element {
	return ElementType(fmt.Sprintf(`<label for="%s" %s>%s</label>`, name, TagMapToElementAttributes(tags, AllTagsLabel...), name))
}

func (i MultipleSelectField) InputHTML(_ *request.Request, name string, tags tags.TagMap) interfaces.Element {
	var b strings.Builder
	b.WriteString(fmt.Sprintf(`<select name="%s" multiple %s>`, name, TagMapToElementAttributes(tags, AllTagsInput...)))
	for _, v := range i {
		b.WriteString(fmt.Sprintf(`<option value="%s">%s</option>`, v, v))
	}
	b.WriteString(`</select>`)
...
func (i CheckBoxField) LabelHTML(_ *request.Request, name string, _ tags.TagMap) interfaces.Element {
	return ElementType(fmt.Sprintf(`<label for="%s">%s</label>`, name, name))
}

func (i CheckBoxField) InputHTML(_ *request.Request, name string, _ tags.TagMap) interfaces.Element {
	var b strings.Builder
	for _, v := range i {
		b.WriteString(fmt.Sprintf(`<input type="checkbox" name="%s" value="%s">`, name, v))
	}
	return ElementType(b.String())
}
